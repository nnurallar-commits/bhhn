...
function renderSettings() {
  ...
  view.innerHTML = `
    ...
    <section class="settings-card" style="display:flex;align-items:center;gap:16px"><img class="logo-settings" src="assets/logo.png" alt="bhhn logosu"><div><h3>bhhn.</h3><p style="margin-bottom:8px">balance between us</p><span class="status ${syncClass}">${esc(syncText)}</span></div></section>
    ...
    <section class="settings-card"><h3>profil</h3><p>Şu an ${esc(member(currentUserId).name)} olarak görünüyorsun.</p><div class="settings-actions"><button class="btn btn-secondary" type="button" data-profile-switch>${cloud.status==='cloud'?'profil / çıkış':'profili değiştir'}</button>${cloud.status==='cloud'?`<button class="btn btn-secondary" type="button" data-space-switch>alan değiştir</button>`:''}</div></section>
